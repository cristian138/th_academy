...
export const Sidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { path: '/integration', label: 'Integración', icon: ArrowRightLeft, roles: ['admin', 'superadmin'] }
  ];
  ...
  return (
    <>
      ...
      <aside 
        className={`
          w-72 bg-brand-navy min-h-screen text-white fixed left-0 top-0 flex flex-col z-50
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 text-white/80 hover:bg-white/10 rounded-sm lg:hidden"
        >
          <X size={24} />
        </button>
